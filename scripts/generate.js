...
module.exports =  function outputJSON (totalItems, depth){
    ...
    var stream = through.obj(function(file, enc, callback) {
         ...
         */
        var obj = function (id, indent, parent){
            this.id = id;
            this.parent = parent;
            this.indent = indent;
         ...
         */
        var counter = 0;
        var buildJSON = function redo (level, indent, parent){
            if(counter < totalItems){
                ...
                for(var i  = 0; i < iteration; i++){
                    var thisObj = new obj(counter, indent, parent);
                    var parent = counter;
                    counter++;
                    if( level > 0 && iteration != remainingItems) {
                        thisObj.children = redo (level-1, indent+1, parent);
                    }   
